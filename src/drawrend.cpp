...
void DrawRend::rasterize_triangle( float x0, float y0,
  ...
  // Part 6: Pass in correct barycentric differentials to tri->color for mipmapping.
    vector<Vector2D> vertexes = {Vector2D(x0,y0),Vector2D(x1,y1),Vector2D(x2,y2)};
    
    //clockwise
    sort(vertexes.begin(), vertexes.end(), compare);
    
    vector<Vector2D> edges = {vertexes[1] - vertexes[0], vertexes[2] - vertexes[1], vertexes[0] - vertexes[2]};
    vector<Vector2D> normals = {edges[0].normal(), edges[1].normal(), edges[2].normal()};
    
    if(dot(normals[0], edges[1])< 0)
        normals[0] *= -1.0;
    if(dot(normals[1], edges[2])< 0)
        normals[1] *= -1.0;
    if(dot(normals[2], edges[0])< 0)
        normals[2] *= -1.0;
    
    /*
    //part 1 naive implementation
    double minY = vertexes[0].y;
    double maxY = vertexes[2].y;
    double minX = min(min(vertexes[0].x, vertexes[1].x), vertexes[2].x);
    double maxX = max(max(vertexes[0].x, vertexes[1].x), vertexes[2].x);
    double offset = 0.5;
    for(int curX = floor(minX); curX <= floor(maxX); curX ++){
        for(int curY = floor(minY); curY <= floor(maxY); curY ++){
            if(isInside(curX + offset, curY + offset, vertexes, normals)){
                rasterize_point(curX, curY, color);
            }
        }
    }
    //end part 1 naive implementation
    */
    
    /*
    //part 2 naive implementation
    double samples_per_side = sqrt(sample_rate);
    double minY = vertexes[0].y;
    double maxY = vertexes[2].y;
    double minX = min(min(vertexes[0].x, vertexes[1].x), vertexes[2].x);
    double maxX = max(max(vertexes[0].x, vertexes[1].x), vertexes[2].x);
    double step = 1.0 / samples_per_side;
    double offset = 0.5 / samples_per_side;
    
    for(int curX = floor(minX); curX <= floor(maxX); curX ++){
        for(int curY = floor(minY); curY <= floor(maxY); curY ++){
            for(int i = 0; i < samples_per_side; i ++){
                for(int j = 0; j < samples_per_side; j ++){
                    if(isInside(curX + i * step + offset, curY + j * step + offset, vertexes, normals)){
                        if ( curX < 0 || curX >= width ) continue;
                        if ( curY < 0 || curY >= height ) continue;
                        samplebuffer[curY][curX].fill_color(j, i, color);
                    }
                }
            }
    ...
    if(tri){
//        cout<<tri->p0_svg.x<<" "<<tri->p0_svg.y<<" "<<tri->p1_svg.x<<" "<<tri->p1_svg.y<<" "<<tri->p2_svg.x<<" "<<tri->p2_svg.y<<" "<<endl;
//        cout<<x0<<" "<<y0<<" "<<x1<<" "<<y1<<" "<<x2<<" "<<y2<<" "<<endl;
        for(int curX = floor(minX); curX <= floor(maxX); curX ++){
            for(int curY = floor(minY); curY <= floor(maxY); curY ++){
                for(int i = 0; i < samples_per_side; i ++){
                    for(int j = 0; j < samples_per_side; j ++){
                        double xp = curX + i * step + offset, yp = curY + j * step + offset;
                        if(isInside(xp, yp, vertexes, normals)){
                            if ( curX < 0 || curX >= width ) continue;
                            if ( curY < 0 || curY >= height ) continue;
                            Vector3D p_bary(1,1,1);
                            //part 4
//                            samplebuffer[curY][curX].fill_color(j, i, tri->color(get_p_bary(x0, y0, x1, y1, x2, y2, xp, yp)));
                            //part 5
                            Vector3D col = get_p_bary(x0, y0, x1, y1, x2, y2, xp, yp);
                            SampleParams sp;
                            sp.psm = psm;
                            sp.lsm = lsm;
//                            samplebuffer[curY][curX].fill_color(j, i, tri->color(col, Vector3D(0,0,0), Vector3D(0,0,0), sp));
                            //part 6
                            Vector3D p_dx_bary = get_p_bary(x0, y0, x1, y1, x2, y2, xp + step, yp);
                            Vector3D p_dy_bary = get_p_bary(x0, y0, x1, y1, x2, y2, xp, yp + step);
                            samplebuffer[curY][curX].fill_color(j, i, tri->color(col, p_dx_bary, p_dy_bary, sp));
                        }
                    }
                }
            }
    ...
    Vector3D DrawRend::get_p_bary(float x0, float y0, float x1, float y1, float x2, float y2, float xp, float yp){
        double alpha = (-(xp - x1)*(y2 - y1) + (yp - y1)*(x2 - x1)) / (-(x0 - x1)*(y2 - y1) + (y0 - y1)*(x2 - x1));
        double beta = (-(xp - x2)*(y0 - y2) + (yp - y2)*(x0 - x2)) / (-(x1 - x2)*(y0 - y2) + (y1 - y2)*(x0 - x2));
        return Vector3D(alpha, beta, 1 - alpha - beta);
    }
    
    bool DrawRend::isInside(double x, double y, vector<Vector2D>& vertexes, vector<Vector2D>& normals){
        Vector2D p(x,y);
        return isInside(p, vertexes, normals);
    }

    bool DrawRend::isInside(Vector2D& p, vector<Vector2D>& vertexes, vector<Vector2D>& normals){
        if(dot(normals[0], p - vertexes[0]) >= 0 &&
           dot(normals[1], p - vertexes[1]) >= 0 &&
           dot(normals[2], p - vertexes[2]) >= 0)
            return true;
//        if(dot(normals[0], p - vertexes[0]) == 0)
        return false;
    }
